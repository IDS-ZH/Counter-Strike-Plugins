...
public Plugin myinfo =
{
    ...
    author      = "ZloyHohol+Gemini (адаптация)",
...
void StopRulesTimer()
{
    ...
    void LoadRules()
    {
        ...
        if (file != null)
        {
            if (ReadFileString(file, buffer, sizeof(buffer)) > 0)
            {
                char[][] parts = new char[32][256];
                int count = ExplodeString(buffer, ";", parts, 32, 256);
                for (int i = 0; i < count; i++)
                {
                    TrimString(parts[i]);
                    if (parts[i][0] != '\0')
                    {
                        g_hPlayerRules.PushString(parts[i]);
                    }
                }
            }
            delete file;
        ...
        {
            LogMessage("[PRD] Player rules file not found: %s", path);
        }
        ...
        if (file != null)
        {
            if (ReadFileString(file, buffer, sizeof(buffer)) > 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (parts[i][0] != '\0')
                    {
                        g_hAdminRules.PushString(parts[i]);
                    }
                }
            }
            delete file;
        ...
        {
            LogMessage("[PRD] Admin rules file not found: %s", path);
        }
    }
...
int FindMVPByStars(int winning_team)
{
    ...
    for (int i = 1; i <= MaxClients; i++)
    {
        ...
        int currentMvpCount = CS_GetMVPCount(i);
        if (currentMvpCount > bestMvpCount)
        {
            bestMvpCount = currentMvpCount;
            bestClient = i;
            bestJoinTime = g_fJoinTime[i];
        }
        else if (currentMvpCount > 0 && currentMvpCount == bestMvpCount)
        {
            // В случае ничьей по звездам, MVP становится тот, кто раньше зашел на сервер
    ...
    // Убедимся, что у лучшего кандидата действительно есть хотя бы одна звезда MVP
    if (bestClient != -1 && CS_GetMVPCount(bestClient) > g_iPrevMvpStars[bestClient])
    {
        return bestClient;
    }
...
void ApplyMvpRewardVotes(int addedVotes)
{
    ...
    int money = GetEntData(g_iMVP, m_iAccount);
    int accountCap = GetAccountCap();
    ...
    if (reward > 0)
    {
        SetEntData(g_iMVP, m_iAccount, money + reward);
...
void StartMvpVote(int winningTeam)
{
    ...
    for (int i = 1; i <= MaxClients; i++)
    {
        if (IsValidClient(i) && GetClientTeam(i) == votingTeam && i != g_iMVP)
        {
            menu.Display(i, 15);
            displayed = true;
        }
