...
{
	name = "ZH Basic Ban Commands",
	author = "AlliedModders LLC & ZH-sys Team",
	description = "Basic Banning Commands (ZH-sys Integrated)",
...
public Action Command_BanIp(int client, int args)
{
	...
	int hit_client = -1;
	// ZH-sys: Using ZH_IsValidClient, though ProcessTargetString does most work. 
	// Main check here is IsFakeClient and CanUserTarget.
	// We can use ZH_IsValidClient to verify existence.
	if (found_client != -1
		&& ZH_IsValidClient(found_client, false, true) // Check bot? No, logic wants !IsFakeClient. ZH_IsValidClient(..., false, true) means check if bot. 
		// Actually ZH_IsValidClient(client, false, false) returns true if valid and connected.
		// Logic was: !IsFakeClient(found_client).
		// ZH_IsValidClient(..., ..., true) returns FALSE if it is a bot? No, wait.
		// ZH_IsValidClient(client, alive, bot) -> checks IsFakeClient if bot=true?
		// Let's re-read zh_core.inc: "If true, checks IsFakeClient()". Usually implies "fails if bot" or "fails if NOT bot"?
		// "checks IsFakeClient()" -> usually means "Exclude bots". 
		// Let's assume ZH_IsValidClient(client, false, true) means "Must result true if NOT a bot".
		// I will check zh_core.sp implementation later if needed, but standard is "Check if not bot".
		/*
		 * @param checkBot      If true, checks IsFakeClient().
		 */
		// So if checkBot is true, and IsFakeClient is true, it probably returns false.
		// So ZH_IsValidClient(found_client, false, true) == !IsFakeClient(found_client) && IsClientConnected(found_client).
